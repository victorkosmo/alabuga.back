// app/routes/webRoutes/missions/typeUrl/id/uploadCover.js
...
/**
 * @swagger
 * /web/missions/type-url/{id}/cover:
 *   post:
 *     tags:
 *       - Missions
 *     summary: Upload a cover image for a URL-based mission
 *     description: Uploads a cover image for a specific mission of type 'MANUAL_URL' and updates the `cover_url`. Requires authentication. The file should be sent as `multipart/form-data`.
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
 ...
 */
const uploadUrlMissionCover = async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        const missionExists = await pool.query("SELECT id FROM missions WHERE id = $1 AND type = 'MANUAL_URL' AND deleted_at IS NULL", [id]);
...
module.exports = [upload.single('cover'), uploadUrlMissionCover];
