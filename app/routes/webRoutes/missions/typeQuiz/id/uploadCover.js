// app/routes/webRoutes/missions/typeQuiz/id/uploadCover.js
...
/**
 * @swagger
 * /web/missions/type-quiz/{id}/cover:
 *   post:
 *     tags:
 *       - Missions
 *     summary: Upload a cover image for a quiz-based mission
 *     description: Uploads a cover image for a specific mission of type 'QUIZ' and updates the `cover_url`. Requires authentication. The file should be sent as `multipart/form-data`.
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
 ...
 */
const uploadQuizMissionCover = async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        const missionExists = await pool.query("SELECT id FROM missions WHERE id = $1 AND type = 'QUIZ' AND deleted_at IS NULL", [id]);
...
module.exports = [upload.single('cover'), uploadQuizMissionCover];
